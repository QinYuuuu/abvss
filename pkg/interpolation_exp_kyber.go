...
// InterpolationExpAtZeroKyber
// input: (x_i, g^(y_i))
// return: g^(y_0), error
func InterpolationExpAtZeroKyber(x []int64, y []kyber.Point, group kyber.Group) (kyber.Point, error) {
	if len(x) != len(y) {
...
func InterpolationExpAtIndexKyber(x []int64, y []kyber.Point, group kyber.Group, index kyber.Scalar) (kyber.Point, error) {
	if len(x) != len(y) {
		return nil, fmt.Errorf("x and y not same length")
	}
	n := len(x)
	...
	xScalers := make([]kyber.Scalar, n)
	for i := 0; i < n; i++ {
		xScalar := group.Scalar().SetInt64(x[i])
		if xScalar.String() == index.String() {
			return y[i], nil
		}
		xScalers[i] = xScalar
	}
	...
	for j := 0; j < n; j++ {
		...
		for i := 0; i < n; i++ {
			...
			// L_j(0) = Π (k≠j) [ x_k - index / (x_k - x_j) ]
			denominator := group.Scalar().Sub(xi, xj)
			if x[i] == x[j] {
				return nil, fmt.Errorf("calculate L_%d(0): x_%d (%f) - x_%d (%f) == 0", j, i, xi, j, xj)
			}
			numerator := group.Scalar().Sub(xi, index)
			ljAtIndex = group.Scalar().Mul(ljAtIndex, group.Scalar().Div(numerator, denominator))
		}
		tmp := group.Point().Mul(ljAtIndex, yj)
		estimatedXAtYZero = group.Point().Add(estimatedXAtYZero, tmp)
	}
	return estimatedXAtYZero, nil
}
